...
void rebase_tree(NODE **node, FILE *log_file)
{
    ...
    char new_question[MAX_LINE_LENGTH];
    char correct_answer[MAX_LINE_LENGTH];
...
void manual_tree_filling(NODE **root, FILE *log_file){
    ...
    extension = (char*) malloc(MAX_LINE_LENGTH * sizeof (char));
    ...
    while(strcmp(extension, "Yes\n") == 0) {
        ...
        fflush(stdin);
        fgets(extension, MAX_LINE_LENGTH, stdin);

        get_input(extension, "Yes\n", "No\n");
